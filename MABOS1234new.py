...
class BANDIT():
    ...
    def HHO(self):
        ...
        while self.t < self.Max_iter:
            ...
            if np.mod(self.t, self.K) == 1:
                ...
                ucb = self.Q+self.c*term
                print("UCB: ", ucb)
                ind = np.random.choice(3, 1, p=[self.soft(ucb, 0), self.soft(ucb,1), self.soft(ucb,2), self.soft(ucb,3)])
                self.count[ind] +=1
                if ind == 1:
                    self.DE()
                elif ind ==2:
    ...
    def WOA(self):
        ...
        while self.t < self.Max_iter:
            ...
            if np.mod(self.t, self.K) == 1:
                ...
                ucb = self.Q+self.c*term
                print("UCB: ", ucb)
                ind = np.random.choice(4, 1, p=[self.soft(ucb, 0), self.soft(ucb,1), self.soft(ucb, 2),self.soft(ucb, 3)])
                self.count[ind] +=1
                if ind == 1:
                    self.DE()
                elif ind == 0:
    ...
    def FFA(self):
        ...
        ##ns = numpy.zeros((self.SearchAgents_no, self.dim))
        ##for i in range(self.dim):
             ##ns[:, i] = numpy.random.uniform(0, 1, self.SearchAgents_no) * (self.ub[i] - self.lb[i]) + self.lb[i]
        #Lightn = np.ones(self.SearchAgents_no)*np.inf
        ...
        for self.t in range(0, self.Max_iter):  # start iterations
            ...
            for i in range(0, self.SearchAgents_no):
                self.fitness[i] = self.objf(self.Positions[i, :])
                #666Lightn[i] = self.objf(self.Positions[i, :])
                

            # Ranking fireflies by their light intensity/objectives

            self.fitness = np.sort(self.objf(self.Positions[i, :]))
            Index = np.argsort(self.objf(self.Positions[i, :]))
